...
export async function POST(request){
    try {
       ...
       const validatonSchema = zSchema.pick({
        email: true
       }).extend({
        password: z.string()
       })

       const validatedData = validatonSchema.safeParse(payload)
       ...
       if(!getUser){
          return response(false, 400, 'Invalid login credential.')
       }
       ...
       if(!getUser.isEmailVerified){
             const secret = new TextEncoder().encode(process.env.SECRET_KEY)
                    ...
                    await sendMail('Email Verification request from Vipin Singh', email, emailVerificationLink(`${process.env.NEXT_PUBLIC_BASE_URL}/auth/verify-email/${token}`))
